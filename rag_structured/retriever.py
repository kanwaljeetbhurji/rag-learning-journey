...
def load_medications(json_path):
...
def retrieve_structured_context(query, json_path, top_k=4):
    ...
    for record in all_records:
        # Flatten all field values into a searchable string
        searchable_parts = [
            record.get("name", ""),
            record.get("category", ""),
        ...
        # Match if any query token exists in record tokens
